...
sh ${SIM_PLATFORM_HOME}/appshell/${SIM_RELEASE}/future/stop_future_exchange_mdreceiver.sh >> crontab.log

# 2) start future exchange mdreceiver
echo "start start_future_exchange_mdreceiver.sh ... "
sh ${SIM_PLATFORM_HOME}/appshell/${SIM_RELEASE}/future/start_future_exchange_mdreceiver.sh >> crontab.log
