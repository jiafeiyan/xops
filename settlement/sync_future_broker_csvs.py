...
def sync_future_broker_csvs(context, conf):
    result_code = 0
    logger = log.get_logger(category="SyncfutureBrokerCsvs")
    ...
    logger.info("[sync future broker csvs with %s] begin" % json.dumps(rsync_config, encoding="UTF-8", ensure_ascii=False))
    try:
        rsync.rsync_groups(context, rsync_config)
    except Exception as e:
        logger.info("[sync future broker csvs with %s] Error: %s" % (
        json.dumps(rsync_config, encoding="UTF-8", ensure_ascii=False), e))
        result_code = -1

    logger.info("[sync future broker csvs with %s] end" % json.dumps(rsync_config, encoding="UTF-8", ensure_ascii=False))
    return result_code
...
def main():
    ...
    process_assert(sync_future_broker_csvs(context, conf))
