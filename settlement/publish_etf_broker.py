...
def publish_etf(context, conf):
    ...
    logger.info("[publish etf broker %s] begin" % (json.dumps(conf, encoding="UTF-8", ensure_ascii=False)))
    try:
        ...
        for settlement_groups in conf.get("settlementGroups"):
            ...
            row = cursor.fetchone()
            if row is None:
                logger.error("[publish etf broker] Error: There is no data for %s-%s."
                             % (settlement_group_id, settlement_id))
                result_code = -1
            elif row[3] == '0':
                logger.error("[publish etf broker] Error: Settlement for %s-%s has not done."
                             % (settlement_group_id, settlement_id))
                result_code = -1
            elif row[3] == '2':
                logger.error("[publish etf broker] Error: Settlement for %s-%s has been published."
                             % (settlement_group_id, settlement_id))
                result_code = -1
            else:
                ...
                sql = """UPDATE siminfo.t_investorfund t1,
                                        (SELECT
                                            ...
                                            t1.available = t1.balance + t2.available - t2.transfee - t2.DelivFee + t2.stockvalue - t2.positionmargin,
...
def main():
    ...
    process_assert(publish_etf(context, conf))
