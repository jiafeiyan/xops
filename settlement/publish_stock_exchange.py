...
def publish_stock(context, conf):
    result_code = 0
    logger = log.get_logger(category="PublishStockExchange")
    ...
    logger.info("[publish stock exchange %s] begin" % (json.dumps(conf, encoding="UTF-8", ensure_ascii=False)))
    try:
        ...
        logger.error("[publish stock exchange] Error: %s" % (e))
        result_code = -1
    finally:
        mysql_conn.close()
    logger.info("[publish stock exchange] end")
    return result_code
