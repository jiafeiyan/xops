...
python ${SIM_PLATFORM_HOME}/settlement/snap_settle_data.py -conf settlement/snap_settle_data_stock.json
if [ $? != 0 ]; then
...
python ${SIM_PLATFORM_HOME}/settlement/snap_settle_data.py -conf settlement/snap_settle_data_etf.json
if [ $? != 0 ]; then
...
python ${SIM_PLATFORM_HOME}/settlement/settle_stock.py -conf settlement/settle_stock_sse.json
if [ $? != 0 ]; then
        echo "settle_stock sse error..."
        exit 1
fi
python ${SIM_PLATFORM_HOME}/settlement/settle_stock.py -conf settlement/settle_stock_szse.json
if [ $? != 0 ]; then
...
python ${SIM_PLATFORM_HOME}/settlement/snap_initial_data.py -conf settlement/snap_initial_data_stock.json
if [ $? != 0 ]; then
...
python ${SIM_PLATFORM_HOME}/settlement/snap_initial_data.py -conf settlement/snap_initial_data_etf.json
if [ $? != 0 ]; then
