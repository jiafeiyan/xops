...
from shfe_trader_handler import SHFETraderHandler
...
def start_trader_service(context, conf):
    logger = log.get_logger(category="SHFETraderService")

    logger.info("[start shfe trader service with %s] begin" % (json.dumps(conf, encoding="UTF-8", ensure_ascii=False)))
    ...
    trader_handler = SHFETraderHandler(trader_api, user_id, password)
