        ...
        if _type in ("order", "withdrawal"):
            try:
                ...
                if _type == "order":
                    ...
                    order_action = shfetraderapi.CShfeFtdcOrderActionField()
                    order_action.UserID = str(user)
                    order_action.ClientID = str(data.get("ClientID"))
                    order_action.ParticipantID = str(data.get("ParticipantID"))
                    order_action.OrderSysID = str(data.get("OrderSysID"))
                    # TORA_TSTP_AF_Delete
                    order_action.ActionFlag = ord('0')
                    request_id = self.handler.get_request_id()
                    self.handler.trader_api.ReqOrderAction(order_action, request_id)
                    self.count += 1
